...
const WeatherCard = ({
  ...
  weather,
    ...
    const WEATHER_API_KEY = 'e53b3951c2d942c0b1473307230805'
    
    useEffect(() => {
        const fetchWeatherInfo = async () => {
            try {
                const response = await fetch(`http://api.weatherapi.com/v1/current.json?key=${WEATHER_API_KEY}&q=Danang`);
    ...
    const getDate = (weatherInfo) => {
        const date = dayjs(weatherInfo.location.localtime).toDate().toString().split(' ')
        return (
    ...
    const renderTodayTag = (weatherInfo) => {
        if (dayjs(weatherInfo.location.localtime).format('DD MMMM YYYY').toString() === dayjs().format('DD MMMM YYYY').toString()) {
            return (
                <Block 
                    flex={0}
    ...
    return (
        <Block card paddingVertical={sizes.xs} marginVertical={sizes.s} color={colors.light} minHeight={75}>
            {weatherInfo && (
